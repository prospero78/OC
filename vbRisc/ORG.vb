...
' MODULE ORG;
Public Module ORG
   ' (* NW  18.4.2016 / 17.9.2016  code generator in Oberon-07 for RISC*)
   ...
   ' IF x.mode = ORB.Const_ THEN x.r := 15 - x.a*8
   ' ELSE load(x);
   ...
   ' BEGIN x.mode := ORB.Const_; x.type := typ; x.a := val
   ' END MakeConstItem;

   ' PROCEDURE MakeRealItem*(VAR x: Item; val: REAL);
   ' BEGIN x.mode := ORB.Const_; x.type := ORB.realType; x.a := SYSTEM.VAL(LONGINT, val)
   ' END MakeRealItem;

   ' PROCEDURE MakeStringItem*(VAR x: Item; len: LONGINT);
   ' (*copies string from ORS-buffer to ORG-string array*)
   ' VAR i: LONGINT;
   ' BEGIN x.mode := ORB.Const_; x.type := ORB.strType; x.a := strx; x.b := len; i := 0;
   ...
   ' ELSIF (y.class = ORB.Const_) & (y.type.form = ORB.String) THEN x.b := y.lev  (*len*)
   ' ELSE x.r := y.lev
   ' END ;
   ' IF (y.lev > 0) & (y.lev # curlev) & (y.class # ORB.Const_) THEN ORS.Mark("level error, not accessible") END
   ' END MakeItem;
   ...
   ' IF (y.mode = ORB.Const_) & (lim >= 0) THEN
   ...
   ' BEGIN
   ' IF op = ORS.plus THEN
   ' IF (x.mode = ORB.Const_) & (y.mode = ORB.Const_) THEN x.a := x.a + y.a
   ' ELSIF y.mode = ORB.Const_ THEN load(x);
   ...
   ' BEGIN
   ' IF (x.mode = ORB.Const_) & (y.mode = ORB.Const_) THEN x.a := x.a * y.a
   ' ELSIF (y.mode = ORB.Const_) & (y.a >= 2) & (log2(y.a, e) = 1) THEN load(x); Put1(Lsl, x.r, x.r, e)
   ' ELSIF y.mode = ORB.Const_ THEN load(x); Put1a(Mul, x.r, x.r, y.a)
   ' ELSIF (x.mode = ORB.Const_) & (x.a >= 2) & (log2(x.a, e) = 1) THEN load(y); Put1(Lsl, y.r, y.r, e); x.mode := Reg; x.r := y.r
   ' ELSIF x.mode = ORB.Const_ THEN load(y); Put1a(Mul, y.r, y.r, x.a); x.mode := Reg; x.r := y.r
   ...
   ' BEGIN
   ' IF op = ORS.div THEN
   ' IF (x.mode = ORB.Const_) & (y.mode = ORB.Const_) THEN
   ' IF y.a > 0 THEN x.a := x.a DIV y.a ELSE ORS.Mark("bad divisor") END
   ' ELSIF (y.mode = ORB.Const_) & (y.a >= 2) & (log2(y.a, e) = 1) THEN load(x); Put1(Asr, x.r, x.r, e)
   ' ELSIF y.mode = ORB.Const_ THEN
   ...
   ' ELSIF (y.mode = ORB.Const_) & (y.a >= 2) & (log2(y.a, e) = 1) THEN load(x);
   ...
   ' BEGIN
   ' IF x.mode = ORB.Const_ THEN x.a := LSL(1, x.a)
   ...
   ' ELSE
   ' IF (x.mode = ORB.Const_) & (x.a <= 16) THEN x.a := LSL(-1, x.a)
   ' ELSE load(x); Put1(Mov, RH, 0, -1); Put0(Lsl, x.r, RH, x.r)
   ' END ;
   ' IF (y.mode = ORB.Const_) & (y.a < 16) THEN Put1(Mov, RH, 0, LSL(-2, y.a)); y.mode := Reg; y.r := RH; incR
   ...
   ' BEGIN load(y);
   ' IF x.mode = ORB.Const_ THEN Put1(Ror, y.r, y.r, (x.a + 1) MOD 20H); DEC(RH)
   ...
   ' BEGIN
   ' IF (y.mode = ORB.Const_) & (y.type.form # ORB.Proc) THEN
   ' load(x);
   ...
   ' BEGIN load(x);
   ' IF (y.mode = ORB.Const_) & (y.a = 0) THEN DEC(RH)
   ...
   ' BEGIN 
   ' IF z.mode = ORB.Const_ THEN Put1a(Cmp, RH, y.r, z.a)
   ...
   ' IF y.type.form = ORB.NoTyp THEN y.mode := ORB.Const_; y.a := 1 END ;
   ' IF (x.mode = ORB.Var) & (x.r > 0) THEN
   ' zr := RH; Put2(Ldr+v, zr, SP, x.a); incR;
   ' IF y.mode = ORB.Const_ THEN Put1a(op, zr, zr, y.a) ELSE load(y); Put0(op, zr, zr, y.r); DEC(RH) END ;
   ...
   ' IF y.mode = ORB.Const_ THEN Put1a(op, zr, zr, LSL(1, y.a))
   ...
   ' BEGIN
   ' IF y.mode = ORB.Const_ THEN Put1a(Mov, x.a, 0, y.a)
   ...
   ' BEGIN
   ' IF x.mode = ORB.Const_ THEN x.a := ABS(x.a)
   ' ELSE load(x);
   ...
   ' x.mode := ORB.Const_; x.a := x.type.len
   ...
   ' IF y.mode = ORB.Const_ THEN Put1(op, x.r, x.r, y.a MOD 20H)
   ...
   ' IF y.mode = ORB.Const_ THEN Put1(Ror, x.r, x.r, y.a+1); DEC(RH)
   ...
   Public Sub Adr()
      ...
      ' ELSIF (x.mode = ORB.Const_) & (x.type.form = ORB.Proc) THEN load(x)
      ' ELSIF (x.mode = ORB.Const_) & (x.type.form = ORB.String) THEN loadStringAdr(x)
   ...
   Public Sub Close()
      ...
      ' ELSIF (obj.exno # 0) & (obj.class = ORB.Const_) & (obj.type.form = ORB.Proc)
      ...
      ' IF (obj.exno # 0) & (obj.class = ORB.Const_) & (obj.type.form = ORB.Proc) &
      ...
      ' IF obj.exno # 0 THEN
      ' IF (obj.class = ORB.Const_) & (obj.type.form = ORB.Proc) OR (obj.class = ORB.Var) THEN
