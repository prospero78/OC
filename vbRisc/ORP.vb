...
' MODULE ORP;
Public Module ORP
   ' (*N. Wirth 1.7.97 / 17.10.2016  Oberon compiler for RISC in Oberon-07*)
        ...
        ' BEGIN (*sync*) pbsList := NIL;
          ...
          ' IF sym = ORS.const THEN
            ...
            ' WHILE sym = ORS.ident DO
              ...
              ' ORB.NewObj(obj, id, ORB.Const_); obj.expo := expo;
              ' IF x.mode = ORB.Const_ THEN obj.val := x.a; obj.lev := x.b; obj.type := x.type
      ...
      ' PROCEDURE ProcedureDecl;
        ...
        ' BEGIN (* ProcedureDecl *) int := FALSE; ORS.Get(sym);
          ...
          ' IF sym = ORS.ident THEN
            ...
            ' ORB.NewObj(proc, ORS.id, ORB.Const_); parblksize := 4;
